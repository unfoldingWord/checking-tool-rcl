...
function generateDataSegment(label, text, isFormatted = false, fontSize = '1.0em') {
  return (isFormatted ?
    <div><Typography component='strong'>{label}</Typography> <Typography component='span' sx={{ fontSize }}>{(text && getFormatted(text)) || ''}</Typography></div>
    :
    <div><Typography component='strong'>{label}</Typography> <Typography component='span' sx={{ fontSize }}>{text}</Typography></div>
  );
}
...
function generateWordEntry(multipart, word, fontSize = '1.2em') {
  return multipart ?
    <div style={{ margin: '0', padding: '0' }}>
      <Typography component='strong' style={{ fontSize }}>{word}</Typography>
      <br/>
    </div>
    : '';
}
