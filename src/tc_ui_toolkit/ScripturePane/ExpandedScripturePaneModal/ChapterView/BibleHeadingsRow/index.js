...
class BibleHeadingsRow extends Component {
  render() {
    const { currentPaneSettings, projectDetailsReducer, bibles } = this.props
    const bibleHeadings = []

    for (let i = 0, len = currentPaneSettings.length; i < len; i++) {
      const { languageId, bibleId, owner, isPreRelease, description } =
        currentPaneSettings[i]
      const bible = getBibleElement(bibles, languageId, bibleId, owner)
      const index = i
      const { language_name, direction } = bible?.manifest || {}

      const resourceText =
        bibleId !== 'targetBible'
          ? ` (${bibleId.toUpperCase()}) (${owner})`
          : ''
      ...
      if (!dir) {
        dir =
          projectDetailsReducer?.manifest?.target_language?.direction || 'ltr'
      }
    ...
    return <Row style={rowStyle}>{bibleHeadings}</Row>
  }
}
