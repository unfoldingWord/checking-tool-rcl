  ...
  render() {
    ...
    if (currentPaneSettings.length > 0) {
      for (let i = 0, len = currentPaneSettings.length; i < len; i++) {
        ...
        try {
          let { font, bibleId, fontSize, languageId, owner } = paneSetting
          ...
          if (bible) {
            const verseDataFromBible = getVerseDataFromBible(
              bible,
              chapter,
              currentVerseNumber
            )
            ...
            const { isVerseSpan, isFirstVerse } = isVerseInSpan(
              verseLabel,
              currentVerseNumber
            )
          ...
          console.error(
            `VerseRow: error rendering verse ${JSON.stringify(paneSetting)}`,
            error
          )
        }
      }
    ...
    return <Row style={rowStyle}>{verseCells}</Row>
  }
}
