...
export function getRef(bible, chapter, verse) {
  verse = String(verse)
  let keys_verse = Object.keys(bible[chapter])
  if (keys_verse.includes(verse)) {
  ...
  if (isVerseSpan(verse)) {
    ...
    for (let e of keys_verse) {
      if (isVerseSpan(e)) {
        const { low, high } = getVerseSpanRange(e)
        if (parseInt(verse) >= low && parseInt(verse) <= high) {
          return { chapter: chapter, verse: e }
        }
      }
    }
  }
